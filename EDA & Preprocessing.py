...
dataset_floats = dataset.drop(['industry','symbol','dps_change_next_year'],axis=1)
top_correlations = rank_columns_by_correlation(dataset_floats, 0.98)
...
categorical_columns = ["industry", "symbol"]
...
column_transformer = ColumnTransformer(
    transformers=[
        ('categorical', OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1),
         # make_column_selector(dtype_exclude=['float64']))
         categorical_columns)
    ],
...
# number like strings
cols_to_convert = {'industry': 'str', 'symbol': 'str', 'year': 'int'}
